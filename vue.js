...
const App = {
  data() {
    return {
      activeIndex: 0,
      btnName: 'Вперед',
      changeBtn: false, // то, что позволяет определить текущий активный шаг
      steps: [{
          title: 'Основы',
  ...
  methods: {
    ...
    nextOfFinish() {
      ...
      if(this.btnName === 'Закончить'){
        return this.changeBtn = true
      }
    },
    setActive(idx) {
  ...
  computed: {
    ...
    isLastStep() {
      if (this.activeIndex === this.steps.length - 1){
        return this.btnName = 'Закончить'
      }else{
        return this.btnName = 'Вперед'
      }
    },
    activeText() {
